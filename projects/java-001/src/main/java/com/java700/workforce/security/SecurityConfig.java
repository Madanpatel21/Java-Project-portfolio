...
/** Stateless JWT security: RBAC matrix at HTTP level + method security for business rules. */
@Configuration
...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, JwtAuthConverter converter) throws Exception {
        http
            .csrf(csrf -> csrf.disable()) // stateless bearer-token API; see threat model
    ...
    /** Dev/test: local identity provider JWT decoder (HS256 shared secret). */
    @Bean
    @Profile({"dev", "test"})
    ...
    /** Argon2id for the local identity provider (dev/fallback). */
    @Bean
    ...
    /** Security event logging: every authentication failure is recorded. */
    @Bean
    ApplicationListener<AbstractAuthenticationFailureEvent> authFailureListener() {
        return event -> log.warn("Authentication failure: principal={}, source={}, remote={}",
