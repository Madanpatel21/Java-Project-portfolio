...
/**
 * Dev/test token endpoint for the built-in local identity provider (Argon2id + lockout).
 * The production-like profile federates to Keycloak instead (OIDC flows).
 */
@RestController
...
public class DevTokenController {
    ...
    ResponseEntity<TokenResponse> token(@Valid @RequestBody TokenRequest request) {
        ...
        JwtClaimsSet claims = JwtClaimsSet.builder()
                ...
                .claim("org_unit", user.getOrgUnit())
                .claim("roles", roles)
                .build();
