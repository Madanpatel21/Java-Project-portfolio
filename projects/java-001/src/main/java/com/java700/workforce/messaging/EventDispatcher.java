...
/** Decodes broker messages and routes them to typed handlers (handlers resolved lazily). */
@Component
public class EventDispatcher {
    ...
    public EventDispatcher(ObjectMapper mapper, ObjectProvider<List<DomainEventHandler<?>>> handlersProvider) {
        this.mapper = mapper;
        handlersProvider.getObject().forEach(h -> byType.put(h.supportedType().getSimpleName(), h));
    }
