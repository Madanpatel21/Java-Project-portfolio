...
/** Records business audit entries in the same transaction as the business write. */
@Service
public class AuditLogService {
    ...
    public void record(String action, String targetType, String targetId, String detail) {
        ...
        repository.save(new AuditLog(
                UUID.randomUUID().toString(), Instant.now(clock), principal, action,
                targetType, targetId, truncate(detail, 2000), MDC.get(CORRELATION_ID)));
    }
